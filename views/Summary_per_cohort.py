...
onerous_contract                = pd.read_csv("streamlit_output_rev/onerous_contract.csv")
onerous_contract['IFRS_MONTH']  = pd.to_datetime(onerous_contract['IFRS_MONTH'], errors='coerce', format='%Y-%m-%d')
...
selected = option_menu(
        ...
        options=["Overall", "Onerous Contract"],
...
if selected == "Overall":
    ...
    if IFRS_MONTH_filter == "All":
        ...
        if round_school(sum(value_chart)) == 0:
            ...
            fig = px.pie(data, values='CSM', names='KEY', title="comparison of CSM in " + IFRS_MONTH_csm.strftime("%B-%Y"))
            st.plotly_chart(fig)
        ...
        fig.update_layout(
            title="Waterfall Chart - CSM Movement in " + IFRS_MONTH_csm.strftime("%B-%Y"),
            showlegend=False,
...
###############################################################################################################################################
if selected == "Onerous Contract":
    
    IFRS_MONTH_onerous_list = list(onerous_contract["IFRS_MONTH"].unique())
    PRODUK_onerous_list     = list(onerous_contract["NAMA_PRODUK"].unique())
    IFRS_MONTH_onerous_list.sort()
    PRODUK_onerous_list.sort()

    IFRS_MONTH_onerous  = st.selectbox("SELECT DATE", IFRS_MONTH_onerous_list)
    PRODUK_onerous      = st.selectbox("SELECT DATE", PRODUK_onerous_list)
    
    # Filter by IFRS_MONTH using a select box
    onerous_df  = onerous_contract[onerous_contract["IFRS_MONTH"] == IFRS_MONTH_onerous]
    onerous_df  = onerous_df[onerous_df["NAMA_PRODUK"] == PRODUK_onerous]
    onerous_df  = onerous_df[["IFRS_MONTH", "NAMA_PRODUK", "KEY", "LIABILITY", "% LIABILITY", "DISTRIBUSI"]]

    onerous_contract_summary    = onerous_contract[["IFRS_MONTH", "NAMA_PRODUK", "LIABILITY", "TOTAL_LIABILITY"]]
    
    onerous_contract_summary                = onerous_contract_summary.groupby(["IFRS_MONTH", "NAMA_PRODUK"], as_index=False).agg({'LIABILITY': 'sum', 'TOTAL_LIABILITY': 'max'})
    perc_liability                          = [x / y * 100 for x, y in zip(onerous_contract_summary['LIABILITY'].tolist(), onerous_contract_summary['TOTAL_LIABILITY'].tolist())]
    perc_liability                          = [f"{p:.2f}" + " %" for p in perc_liability]
    onerous_contract_summary['% LIABILITY'] = perc_liability
    onerous_contract_summary                = onerous_contract_summary[onerous_contract_summary["IFRS_MONTH"] == IFRS_MONTH_onerous]


    st.write(onerous_contract_summary)
    st.write(onerous_df)